...
#---------------------------$$$$$$------------------------------
#Now loading models
generator=Generator(channels_noise=Z_dimension, channels_img=channels, features_g=Generator_features).to(device)
...
#Now initializing our  optimizers here adam will be instead of RMSprop
...
writer_real = SummaryWriter(f"logstate/real")
writer_fake = SummaryWriter(f"logstate/fake")
step = 0
critic_iteration=5
#weight_clip=0.1 #No need here.
generator.train()
critic.train()
for i in range(Num_epochs):
    ...
    for index,(images,_) in enumerate(mnist_loader):
        ...
        for _ in range(critic_iteration):
            ...
            #Now applying gradient penalty here.
            gp=gradient_penalty(critic,images, fake_img,device=device)
            #Succesful critic must be able to differentiate between fake and real
            #Same as previous loss function just added a new term
        ...
        if index%100==0 and index>0:
            ...
            critic.eval()
            print(
               f"Epoch [{i}/{Num_epochs}] Batch {index}/{len(mnist_loader)} \
                 Loss D: {loss_critic:.4f}, loss G: {gen_loss:.4f}"
           )
